...
`/codewise update` 触发时，**必须从本次会话的第一条消息开始，一轮不漏地重新读完整个对话历史**，直到触发 update 的这一刻。不是扫最近几条，不是靠"我还记得"，而是**真的把上下文从头过一遍**——人类的记忆会压缩细节，模型的注意力会偏向近期消息，调试早期的关键线索（最初的报错、被排除的假设、中途的误判）往往就在这时被丢掉，而这些恰恰是 pitfalls 的核心原料。

只要会话里出现过任何开发/调试/讨论痕迹，就必须提取，不存在"没什么可记的"这种结论——如果你觉得没什么可记，八成是没读完。
...
**只提取代码里看不出来的信息。** 代码改了什么、文件结构怎么变的，这些从 git diff 就能看到，不需要从会话里提取——但 git diff **看不到**的根因分析、误判过程、为什么这么改，必须从会话里抢救出来。
