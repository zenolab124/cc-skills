---
name: codewise
description: 显式调用的项目级知识库生成与维护工作流。扫描指定代码 scope、全部 Git Worktree 与项目相关的 Codex、Claude Code、Gemini CLI、OpenCode、Cursor、Aider 等会话，维护被主仓库忽略的独立 Git 知识库 docs/knowledge/。支持首次生成、增量更新、重建、局部刷新和分支知识合并。Use only when the user explicitly invokes "$codewise" or selects Codewise from the Skill picker；不要因纯文本 codewise、/codewise、泛泛的 knowledge、documentation 或 onboarding 请求隐式触发。
---
...
| 参数 | 行为 |
|---|---|
| 无参数 / `update` | Phase 0 解析后的 `<KBR>` 已有 INDEX 时增量更新；根 KB 没有时首次生成 |
...
- [storage-layout.md](references/storage-layout.md)：统一路径、身份、父仓库安全、锁、bootstrap 与跨机器同步。
- [branch-resolution.md](references/branch-resolution.md)：`<KBR>`、分支登记、会话分段与事实采纳。

按模式再读取：

- 首次/rebuild：[detection.md](references/detection.md)、[session-sources.md](references/session-sources.md)、[signal-extraction.md](references/signal-extraction.md)、[entry-templates.md](references/entry-templates.md)、[index-template.md](references/index-template.md)。
- update：`session-sources`、`signal-extraction`；涉及文档/接口时再读 `detection`；写条目/索引时读对应模板。
- refresh：`detection` + `index-template`。
- merge：[merge-protocol.md](references/merge-protocol.md) + `index-template`。
...
不修改其他条目、`baseline_commit` 或 `synced_at`；与其他写模式一样要求 clean/snapshot 不变量。局部刷新不能替代 update。
...
Phase M 不前移根 baseline；完成后提示运行 update。最后 commit、mirror、普通 push。
