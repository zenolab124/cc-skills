...
⚠️ **主线分支名不是 `main`，要从元信息读。** `main/` 是**目录名**（固定），而仓库的主线**分支**可能叫 `master`、`trunk` 或任何名字。首次生成时探测当前分支并写入 `main/_meta.json`：

```json
{ "anchor_kind": "main", "branch": "<首次生成时的当前分支名>" }
```

之后推导时，候选集 = `main/_meta.json` 的 `branch` + 各 `branches/<slug>/_meta.json` 的 `branch`。**任何地方都不要把字符串 `"main"` 当分支名用**——那会在 `master` 仓库上静默失配，退化成"所有分支都算不出归属"。
...
- 没有自己的目录 → 归属推导出的最近祖先；若最近祖先是主线分支，`<KBR>` = `<KB>/<SCOPE>/main`。
- **主线分支已被改名或删除**（`_meta.json` 记的分支不存在）→ 报告并询问，不要猜；确认后更新 `main/_meta.json` 的 `branch`。
...
**把推导结果摆出来让用户确认，不要问"这个分支算长期吗"这种抽象问题。** 选择记入 `<KB>/<SCOPE>/_branches.json`（分支登记表），之后同一分支不再询问。
